...
export function buildReminders(settings: HealthSettings): HealthReminder[] {
  return defaultHealthReminders.map((reminder) => ({
    ...reminder,
...
export function isShiftDay(date: Date, settings: HealthSettings) {
  return settings.shiftDays.includes(date.getDay());
}
...
export function setQuietMode(state: HealthState, minutes: number, now = new Date()): HealthState {
  ...
  return {
    ...state,
    settings: { ...state.settings, quietModeUntil: until },
    days: {
      ...state.days,
      [today.date]: { ...today, urgentTicketModeCount: today.urgentTicketModeCount + 1 }
    }
  };
}
